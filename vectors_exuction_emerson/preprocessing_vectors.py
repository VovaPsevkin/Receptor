...
class Preprocessing(object):
    ...
    def __create_combinations(self):
        ...
        for personal_info, ind in tqdm(self.__id2patient.items(), total=self.__size, desc='Create Combinations',
                                       bar_format="{l_bar}%s{bar}%s{r_bar}" % (Fore.LIGHTBLUE_EX, Fore.RESET)):
            _, _, _, path = personal_info
            for element in pd.read_csv(path, usecols=[3])['projection']:
    ...
    def __filter_dict(self):
        ...
        with open('combinations.pickle', 'wb') as handle:
            pickle.dump(self.combinations, handle, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    def __create_cmv_vector(self):
        ...
        for item, index in self.__id2patient.items():
            if item[1] == 'positive':
                self.cmv[index] = 1
        print("AAA")
        with open('cmv.pickle', 'wb') as handle:
            pickle.dump(self.cmv, handle, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    def activate_mechanism(self, directory):
        ...
        with open('cmv.pickle', 'rb') as handle:
            self.cmv = pickle.load(handle)
        print("CCC", len(self.cmv))

        with open('combinations.pickle', 'rb') as handle:
            self.combinations = pickle.load(handle)
        print("CCC", len(self.combinations))
