...
from vectors_exuction_emerson.preprocessing_vectors import Preprocessing
...
class EmersonSecondVersion(Preprocessing):
    ...
    def prepare_combinations(self, data_path):
        ...
        for path in tqdm(directory.glob('*'), total=size, desc='Create combinations of test data',
                         bar_format="{l_bar}%s{bar}%s{r_bar}" % (Fore.LIGHTBLUE_EX, Fore.RESET)):
            patient_name, _, label, _ = path.stem.split('_')
            data = pd.read_csv(path, usecols=[3])['projection']
    ...
    def fit(self, data_path):
        try:
            ...
            self.filtered_p_value_combinations = EmersonSecondVersion.fishers_exact_test_sort(self.combinations, \
...
if __name__ == '__main__':
    try:
        ...
        DATA = ROOT / 'Data'
        TRAIN = DATA / 'PreprocessedTrain_TFIDF_test'
        TEST = DATA / 'PreprocessedTest_TFIDF_test'
        ...
        _, _, compositions = Path(args.train_data_path).stem.split('_')
